...
<div class="main-panel m-3">
  ...
  <h4>Berikut Adalah Hasil Scraping Dari Data Hotel Best Western</h4>
  <?php

  include "koneksi.php";
  $query = mysqli_query($conn, "SELECT * FROM scarping_data_hotel");
  ...
  $query_paginate = mysqli_query($conn, "SELECT * FROM scarping_data_hotel LIMIT $awal_data, $jumlah_data_per_halaman");
  ...
  <tr>
    ...
    <th>Alamat</th>
    <th>Nearby Place</th>
    <th>Tempat Populer Disekitar</th>
  ...
  while ($data = mysqli_fetch_array($query_paginate)) {
    ...
    <td>" . $data['nearby'] . "</td>
    <td>" . $data['populer'] . "</td>
    <td>" . $data['superiorDouble'] . "</td>
    <td>" . $data['Duluxe Double'] . "</td>
    <td>" . $data['Deluxe Twin'] . "</td>
  </tr>";
    $i++;
  }
  echo "</table>";
